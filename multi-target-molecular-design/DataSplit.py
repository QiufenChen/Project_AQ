...
name = ["smiles", "LogS"]
TrainData = [[smiles[idx], float(labels[idx])] for idx in (train_inds+valid_inds)]
...
mkdir(SavePath)
TrainData_df.to_csv(SavePath + "train.csv", index=False)
TestData_df.to_csv(SavePath + "test.csv", index=False)
