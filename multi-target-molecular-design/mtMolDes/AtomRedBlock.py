...
class AtomRedBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        h = leakyrelu(self.fc_layers[1](h))
        h = self.fc_layers[2](h)
        h = leakyrelu(self.fc_layers[3](h))
        h = self.fc_layers[4](h)
        h = relu(self.fc_layers[5](h))
        h = h.sum(0)  # 1 x num_output

        # h = self.fc_layers[0](h)
        # h = th.tanh(self.fc_layers[1](h))
        # h = self.fc_layers[2](h)
        # h = th.tanh(self.fc_layers[3](h))
        # h = self.fc_layers[4](h)
        # # h = relu(self.fc_layers[2](h))
        # h = self.fc_layers[5](h)
        # h = h.sum(0)  # 1 x num_output

        # h = th.max(h)
        # for layer in self.fc_layers:
        #     h = th.tanh(layer(h))  # num_atoms x num_labels
        # h = h.sum(0)  # 1 x num_output
        # h = th.max(h)
        # h = th.mean(h, dim=0)
